...
def execute():
	fields = [
		{"fieldname": "custom_late_minutes", "label": "Late (min)", "fieldtype": "Int",
		 "insert_after": "late_entry", "non_negative": 1},
		{"fieldname": "custom_early_minutes", "label": "Early leave (min)", "fieldtype": "Int",
		 "insert_after": "custom_late_minutes", "non_negative": 1},
		{"fieldname": "custom_overtime_minutes", "label": "Overtime (min)", "fieldtype": "Int",
		 "insert_after": "custom_early_minutes", "non_negative": 1},
	]
	pending = [f for f in fields if not frappe.db.exists("Custom Field", {"dt": "Attendance", "fieldname": f["fieldname"]})]
	if pending:
