...
def run():
	...
	for ci in sorted(db_cis, key=lambda x: x.ci_number or x.name):
		...
		is_ok = (
			abs(ag_db - ag_ref) <= 0.01
			and abs(dc_db - dc_ref) <= 0.01
			and cin
			and not cin.startswith("CI-2026-")
		)
	...
	print(
		f"Newer / Post-Migration CIs: {len([c for c in ci_report if c['reason'] == 'NOT_IN_LEGACY_SHEET'])}\n"
	)
