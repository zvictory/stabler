...
def run(dry_run=1):
	...
	pis = frappe.get_all(
		"Proforma Invoice",
		fields=["name", "supplier", "supplier_pi_ref", "agreed_total"]
	)
	...
	for pi in pis:
		...
		if expected_ref and (not orig or orig.startswith("PI-2026-") or orig != expected_ref):
			...
			if not dry_run:
				frappe.db.set_value("Proforma Invoice", pi.name, "supplier_pi_ref", expected_ref, update_modified=False)
	...
	for f in fixes:
		print(f"PI: {f['name']:15} | Supplier: {f['supplier']:35} | Old Ref: {f['old_ref']:20} -> New Ref: {f['new_ref']:25} | Agreed: ${f['agreed_total']:12,.2f}")
