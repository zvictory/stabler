...
def run(dry_run=1):
	...
	if not dry_run:
		...
		existing_pinvs = frappe.get_all(
			"Purchase Invoice", filters={"supplier": transporter, "company": company}, fields=["name"]
		)
		for pinv in existing_pinvs:
			frappe.db.sql(
				"DELETE FROM `tabGL Entry` WHERE voucher_type='Purchase Invoice' AND voucher_no=%s",
				pinv["name"],
			)
