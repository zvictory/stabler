	...
	for sheet_name in transporter_sheets:
		...
		for r in range(6, ws.max_row + 1):
			...
			try:
				...
				cost = 0.0
			try:
				bank_pay = float(bank_val) if bank_val is not None else 0.0
			except (TypeError, ValueError):
				bank_pay = 0.0
			try:
				cash_pay = float(cash_val) if cash_val is not None else 0.0
			except (TypeError, ValueError):
				cash_pay = 0.0

			b_date_str = str(bank_date_val)[:10] if bank_date_val else ""
			c_date_str = str(cash_date_val)[:10] if cash_date_val else ""
			if not re.match(r"^\d{4}-\d{2}-\d{2}$", b_date_str):
				b_date_str = ""
			if not re.match(r"^\d{4}-\d{2}-\d{2}$", c_date_str):
				c_date_str = ""
...
def run(dry_run=1):
	...
	for idx, r in enumerate(rows, 1):
		...
		if not frappe.db.exists("Supplier", transporter_name):
			...
			if not dry_run:
				frappe.db.set_value(
					"Supplier", transporter_name, "default_currency", "USD", update_modified=False
				)
		...
		for cnt in containers:
			...
			if found_c:
				...
				if not dry_run and ci_doc_name:
					frappe.db.set_value(
						"Import Container", c_name, "commercial_invoice", ci_doc_name, update_modified=False
					)
			elif not dry_run:
		...
		existing_fb = (
			frappe.get_all("Freight Booking", filters=fb_filters, fields=["name"]) if fb_filters else []
		)
