...
def run():
	...
	hma_pi_refs = [r for r in pi_ref_rows if "HMA" in (r.get("vendor") or "").upper() or "HMA" in (r.get("ref") or "").upper()]
	...
	if not db_pis:
		...
		db_pis = [p for p in db_pis if "HMA" in (p.supplier or "").upper() or "HMA" in (p.supplier_pi_ref or "").upper()]
	...
	for ref_pi in hma_pi_refs:
		...
		is_ok = matched and abs(db_ag - ref_ag) <= 0.01 and abs(db_dc - ref_dc) <= 0.01 and db_orig_ref == ref_num
	...
	hma_ci_refs = [r for r in ci_ref_rows if "HMA" in (r.get("vendor") or "").upper() or "HMA" in (r.get("ci_number") or "").upper()]
	...
	print("=========================================================================")
	print(f"HMA Proforma Invoices (PI): Total Ref = {len(hma_pi_refs)} | 100% OK = {len([p for p in pi_audit if p['is_ok']])}")
	print(f"HMA Commercial Invoices (CI): Total Ref = {len(hma_ci_refs)} | 100% OK = {len([c for c in ci_audit if c['is_ok']])} | Discrepancies = {len(discrepant_cis)}\n")
