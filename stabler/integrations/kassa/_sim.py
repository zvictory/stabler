...
_ALIAS = {
    "kirim": sf.BTN_KIRIM, "chiqim": sf.BTN_CHIQIM, "konv": sf.BTN_KONV,
    "konversiya": sf.BTN_KONV, "k2k": sf.BTN_K2K, "kassalararo": sf.BTN_K2K,
    "ochilish": sf.BTN_OPENING, "opening": sf.BTN_OPENING,
    "ok": sf.BTN_CONFIRM, "ha": sf.BTN_CONFIRM, "tasdiq": sf.BTN_CONFIRM,
    "bekor": sf.BTN_CANCEL, "yoq": sf.BTN_CANCEL, "cancel": sf.BTN_CANCEL,
    "som": sf.BTN_SOM, "pk": sf.BTN_PK, "usd": sf.BTN_USD,
}
...
def main():
    ...
    def apply(action):
        ...
        if action.get("type") == "record":
            ss.add_entry(path, company=COMPANY, kassir="sim", op=action["op"],
                         deltas=action["deltas"], counterparty=action.get("counterparty"),
                         purpose=action.get("purpose"), rate=action.get("rate"),
                         raw_text=action.get("raw_text"), parsed=action.get("parsed"), date=DATE)
