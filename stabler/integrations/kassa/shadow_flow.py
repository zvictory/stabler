...
def handle(state, text, ctx=None):
	...
	if step == "menu":
		if t == BTN_KONV:
			...
			return (
				"Konvertatsiya — yo'nalishni tanlang:",
				KONV_DIR_KEYBOARD,
				{"step": "await_konv_dir"},
				None,
			)
		...
		if t == BTN_OPENING:
			return (
				"Ochilish balansini yozing (masalan: «402 mln naqd, 3 mln karta, 400 dollar»):",
				[[BTN_CANCEL]],
				{"step": "await_opening"},
				None,
			)
	...
	if step == "await_opening":
		...
		if not legs:
			return (
				"Tushunmadim. Masalan: «402 mln naqd, 3 mln karta, 400 dollar»",
				[[BTN_CANCEL]],
				{"step": "await_opening"},
				None,
			)
		...
		return (
			_opening_confirm_text(openings),
			[[BTN_CONFIRM], [BTN_CANCEL]],
			{"step": "confirm_opening", "openings": openings},
			None,
		)
