...
ROWS = {
	...
	"Every number is read from an ERP record": (
		...
		"Каждое число читается из записи ERP",
		"Har bir raqam ERP yozuvidan o'qiladi",
		"Ҳар бир рақам ERP ёзувидан ўқилади",
	),
	"A step is late when its average wait passes the threshold set for that step": (
		"Bir adım, ortalama beklemesi o adımın eşiğini aştığında gecikmiş sayılır",
		"Шаг считается просроченным, когда среднее ожидание превышает его порог",
		"Bosqich o'rtacha kutish o'z chegarasidan oshganda kechikkan hisoblanadi",
	...
	# ── Durumlar ────────────────────────────────────────────────────────
	"Within": ("İçinde", "В пределах", "Ichida", "Ичида"),
	"At the edge": ("Sınırda", "На грани", "Chegarada", "Чегарада"),
	"Over SLA": ("SLA dışı", "Вне SLA", "SLA dan tashqari", "SLA дан ташқари"),
	...
	"SLA": ("SLA", "SLA", "SLA", "SLA"),
	"threshold": ("eşik", "порог", "chegara", "чегара"),
	...
	"average wait past the tenant's threshold": (
		...
		"среднее ожидание превысило порог арендатора",
		"o'rtacha kutish ijarachi chegarasidan oshdi",
		"ўртача кутиш ижарачи чегарасидан ошди",
	),
	"Bottleneck": ("Darboğaz", "Узкое место", "Tor joy", "Тор жой"),
	"none today": ("bugün yok", "сегодня нет", "bugun yo'q", "бугун йўқ"),
	"furthest past its own threshold, proportionally": (
		"kendi eşiğini oransal olarak en çok aşan",
		"наибольшее превышение собственного порога в долях",
		"o'z chegarasidan nisbatan eng ko'p oshgan",
		"ўз чегарасидан нисбатан энг кўп ошган",
	),
	"moved before the stage clock existed — left out of the averages": (
		"aşama saati yokken taşınmış — ortalamalara katılmadı",
		"перемещены до появления счётчика этапа — в средние не вошли",
		"bosqich soati paydo bo'lishidan oldin ko'chirilgan — o'rtachaga kirmadi",
		"босқич соати пайдо бўлишидан олдин кўчирилган — ўртачага кирмади",
	),
	"without a stage stamp — not averaged": (
		"aşama damgası yok — ortalamaya katılmadı",
		"без отметки этапа — не усреднено",
		"bosqich muhri yo'q — o'rtachaga kirmadi",
		"босқич муҳри йўқ — ўртачага кирмади",
	),
	"Thresholds come from Stabler Settings, per company": (
		"Eşikler Stabler Ayarları'ndan, şirket başına gelir",
		"Пороги берутся из настроек Stabler, по компании",
		"Chegaralar Stabler sozlamalaridan, kompaniya bo'yicha",
		"Чегаралар Stabler созламаларидан, компания бўйича",
	),
	"deals": ("anlaşma", "сделок", "bitim", "битим"),
	"step": ("adım", "шаг", "bosqich", "босқич"),
	"steps": ("adım", "шагов", "bosqich", "босқич"),
	"days": ("gün", "дн.", "kun", "кун"),
	"Open": ("Açık", "Открыто", "Ochiq", "Очиқ"),
	"Step": ("Adım", "Шаг", "Bosqich", "Босқич"),
}
