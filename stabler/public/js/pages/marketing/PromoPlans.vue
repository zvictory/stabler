...
async function submitForm() {
	...
	try {
		...
		formError.value = e?.message || t("Failed to create promo plan.");
	} finally {
		saving.value = false;
	}
}
...
<template>
	...
	<div v-if="drawerOpen" class="offcanvas-backdrop fade show" @click="closeCreate"></div>
	<div
	...
	>
		<div class="offcanvas-header">
			<h5 class="offcanvas-title">{{ t("New Promo Plan") }}</h5>
			<button type="button" class="btn-close" @click="closeCreate" :aria-label="t('Close')"></button>
		</div>
		<div class="offcanvas-body">
			...
			<div class="d-flex gap-2 mt-4">
				...
				<button type="button" class="btn btn-link" @click="closeCreate">{{ t("Cancel") }}</button>
			</div>
		</div>
	</div>
</template>
