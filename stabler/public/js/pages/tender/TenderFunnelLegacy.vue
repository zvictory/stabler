<script setup>
/* ⚠️ GEÇİCİ KARŞILAŞTIRMA KOPYASI — kalıcı değil.
 *
 * `TenderFunnel.vue`'nun `2d6ff37` ("migrate the tender funnel to the Modernist
 * Tabler layer", 2026-07-31) ÖNCESİ hali, dokunulmadan. Amaç tek: yeni huniyle
 * yan yana bakılıp hangisinin kalacağına karar verilmesi
 * (`/tender/funnel-compare`).
 *
 * Bootstrap/Tabler sınıfları (`card`, `bg-indigo`) ve sabit hex renkler taşır —
 * `stbl-ds` katmanının İÇİNDE çizilmemeli, o yüzden karşılaştırma sayfası onu
 * ds kabuğunun dışında tutuyor.
 *
 * Karar verilince bu dosya SİLİNİR: ya grafiği yeni bileşene taşırız ya da
 * eskiye döneriz. Hiçbir üretim rotası buna bağlanmamalı.
 */
