...
async function load() {
	...
	loading.value = true;
	try {
		const response = await call("stabler.api.tender_master.list_tender_masters", { company: activeCompany.value });
			...
			<div class="card-body p-0">
				<table class="table card-table">
					...
					<tbody>
						...
						<tr v-for="lot in lots" v-else :key="lot.name" role="button" @click="openLot(lot)"><td class="fw-semibold">{{ lot.name }}</td><td>{{ lot.status || "—" }}</td><td class="text-end font-monospace">{{ money(lot.custom_estimated_value, lot) }}</td><td>{{ lot.modified ? formatDate(lot.modified) : "—" }}</td></tr>
					</tbody>
				</table>
