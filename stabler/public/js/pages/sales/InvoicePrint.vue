...
<template>
	<div class="print-wrapper">
		...
		<div class="no-print mb-3 d-flex gap-2">
			...
			<router-link
				:to="'/sales/invoices/' + route.params.name + '/waybill'"
				class="btn btn-sm btn-outline-secondary ms-auto"
			>
				<i class="ti ti-truck me-1"></i>{{ t("Yuk xati") }}
			</router-link>
			<button type="button" class="btn btn-sm btn-primary" @click="triggerPrint">
		...
		<div v-else-if="doc" class="a5-print">
			...
			<!-- 2. Meta — invoice id + date left, customer right -->
			<div class="inv-meta">
				<div>
				...
				<div class="text-end">
					<div class="inv-cust">{{ doc.customer_name }}</div>
					<div v-if="doc.customer_phone" class="inv-sub">{{ doc.customer_phone }}</div>
				</div>
			</div>
			...
			<!-- 5. Amount in words + balance + remarks -->
			...
			<div class="inv-balance">
				<span>
					{{ t("Mijoz balansi") }}:
					<strong :class="doc.customer_balance > 0 ? 'inv-neg' : 'inv-pos'">{{ fmt(doc.customer_balance) }}</strong>
				</span>
			</div>
