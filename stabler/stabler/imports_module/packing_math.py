...
def reconcile_ci_items(
	ci_rows: Iterable[Mapping[str, object]], packed_rows: Iterable[Mapping[str, object]]
) -> list[dict]:
	...
	packed = {
		str(row.get("item_code") or ""): float(row.get("expected_total_kg") or 0) for row in packed_rows
	}
	reconciliation = []
...
def packing_readiness(
	container_names: Iterable[str],
	containers_with_rows: Iterable[str],
	reconciliation: Iterable[Mapping[str, object]],
) -> str:
