...
def _fetch_open_invoices(customer: str) -> list[dict]:
	...
	children = frappe.get_all("Customer", filters={"custom_parent_customer": customer}, pluck="name")
	if children:
...
def simulate_fifo_multi(rows: list[dict]) -> list[dict]:
	...
	for r in rows:
		if r["amount"] <= 0:
			...
			sims.append({"parsed": r, "allocations": [], "overpayment": r["amount"], "refund": True})
			continue
		...
		for inv in invoices:
			...
			allocations.append(
				{"invoice_name": inv["name"], "allocated": allocated, "customer": inv["customer"]}
			)

		sims.append(
			{"parsed": r, "allocations": allocations, "overpayment": remaining_payment, "refund": False}
		)
		...
		"preview_rows": [
			{
				...
				"date": s["parsed"]["posting_date"].isoformat()
				if hasattr(s["parsed"]["posting_date"], "isoformat")
				else str(s["parsed"]["posting_date"]),
