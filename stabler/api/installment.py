...
def _create_invoice(
	...
	schedule = _build_schedule(
		float(flt(cost)), float(flt(markup)), float(flt(dp_percent)), int(term_months), start_date
	)
		...
		"items": [
			{
				...
				"amount": total,
				**(
					{"income_account": income_expense_account}
					if side == "sell"
					else {"expense_account": income_expense_account}
				),
			}
		],
...
def _party_mobile(party_doctype: str, party: str) -> str:
	...
	if not contact:
		return ""
	return (
		frappe.db.get_value("Contact", contact, "mobile_no")
