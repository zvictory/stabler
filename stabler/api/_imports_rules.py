...
def truck_filter_clauses(search=None, status=None, commercial_invoice=None):
    ...
    if search:
        clauses.append(
            "(tr.truck_number LIKE %(search)s OR tr.name LIKE %(search)s "
            "OR tr.driver_name LIKE %(search)s)"
        )
        params["search"] = f"%{search}%"
    if status:
...
def grn_filter_clauses(search=None, status=None, variance_category=None):
    ...
    if search:
        clauses.append(
            "(g.name LIKE %(search)s OR g.commercial_invoice LIKE %(search)s "
            "OR g.supplier LIKE %(search)s)"
        )
        params["search"] = f"%{search}%"
    if status:
...
def derive_bill_category(
    *, truck_ref=None, expense_ref=None, item_codes=(), bill_no=None
) -> str:
