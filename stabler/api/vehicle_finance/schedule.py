...
def build_schedule(
	*,
	...
	for i in range(1, int(installment_count) + 1):
		rows.append(_row(i, add_months(first_dt, i - 1), per_installment, "Installment"))
	if balloon_amount > 0:
		rows.append(
			_row(
...
def _validate_custom_rows(custom_rows: list[dict], total: float, agreement_dt, precision: int) -> list[dict]:
	if not custom_rows:
