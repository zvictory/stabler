...
def remittance_event_query(user=None):
	...
	Until v92 the event had no ``company`` column and this emitted a subquery
	against the parent transfer — one per list query, plus two extra clauses to
	keep an event with a blank link visible. The column made it the same
	one-liner as the transfer, and deleted a second scoping idiom that had to be
	kept correct alongside this one.
	"""
	return _company_condition(user, "Remittance Event")
