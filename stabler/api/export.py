...
REPORT_EXPORTS: dict[str, dict] = {
	...
	# Report Center function-source reports (reports.py) — columns/totals come
	# from the function's own _shape() output, not declared here.
	"sales_by_salesperson": {
		"title": "Sales by Salesperson",
		"source": "stabler.api.reports.sales_by_salesperson",
		"filename": "Sales_by_Salesperson",
		"roles": None,
		"sensitive": False,
	},
	"sales_orders": {
		"title": "Sales Orders (Booked)",
		"source": "stabler.api.reports.sales_orders",
	...
	"sales_trend": {
		"title": "Sales Trend",
		"source": "stabler.api.reports.sales_trend",
...
def _is_empty(v) -> bool:
	"""None, empty string, or empty list/tuple/dict — but not 0 or False."""
	return v is None or v == "" or (isinstance(v, (list, tuple, dict)) and not v)


def _fmt(v):
	"""Render a filter value for the header's 'Filters' line — lists join, not repr()."""
...
def _call_source(source: str, filters: dict) -> dict:
	...
	kwargs = {k: v for k, v in (filters or {}).items() if k in sig.parameters and not _is_empty(v)}
	return fn(**kwargs) or {}
...
def _header_meta(spec: dict, filters: dict, shaped_meta: dict) -> list:
	...
	meta = [
		...
		("Filters", ", ".join(f"{k}={_fmt(v)}" for k, v in extra.items()) if extra else "—"),
