...
def _supplier_gl_balances(company: str, as_of: str | None = None) -> dict:
	...
	return {r["party"]: {"balance": flt(r["balance_acc"]), "currency": r["account_currency"]} for r in rows}
...
def sales_by_customer(
	...
	if items:
		...
		meta = {
			...
			"to": str(end),
			"note": _(
				"Item filter active — figures are line-level (pre-tax); invoice balance/outstanding is not shown because it cannot be split by item."
			),
	...
	meta = {
		...
		"to": str(end),
		"note": _(
			"Sales = selected period · Balance = receivable as of the To date (changes with the date, QuickBooks-style). Click a customer to see the ledger behind the balance."
		),
		"drill_report": "customer_balance_detail",
		"drill_param": "customer",
	}
...
def agreement_receivables(
	...
	return _shape(
		...
		},
		{
			...
			"note": _(
				"Grouped by Contract. Unlinked invoices are shown separately; transaction and base currency are never mixed."
			),
			"drill_report": "customer_balance_detail",
			"drill_param": "customer",
		},
...
@frappe.whitelist()
def customer_balance_detail(
	company: str, customer: str, from_date: str | None = None, to_date: str | None = None
) -> dict:
...
def item_abc(
	...
	columns = [
		...
		{
			"key": metric,
			"label": _("Sales") if metric == "revenue" else _("Qty"),
			"type": "money" if metric == "revenue" else "number",
			"align": "end",
		},
...
def purchases_by_supplier(
	...
	if items:
		...
		meta = {
			...
			"to": str(end),
			"note": _(
				"Item filter active — figures are line-level (pre-tax); outstanding is not shown because it cannot be split by item."
			),
	...
	rows = _sort_rows(
		rows, sort_by, sort_dir, {"supplier_name", "total", "invoice_count", "balance"}, "total"
	)
	totals = {
		"invoice_count": sum(r["invoice_count"] for r in rows),
		"total": sum(r["total"] for r in rows),
	...
	meta = {
		"basis": _("Accrual (bill date)"),
		"currency": base_ccy,
		"from": str(start),
		"to": str(end),
		"note": _(
			"Purchases = selected period · Balance = payable as of the To date (changes with the date). Click a supplier to see the bills."
		),
		"drill_param": "supplier",
	}
	return _shape(columns, rows, totals, meta)
...
def inventory_expiry(company: str, horizon_days: int = 60, items=None) -> dict:
	...
	meta = {
		**base,
		"note": _("Batches expiring within {0} days are flagged. Value at item valuation rate.").format(
			horizon
		),
	}
...
def _margin_meta(company, start, end, zero_cost):
	...
	if zero_cost:
		note += " " + _(
			"⚠ {0} line(s) had no cost — verify item valuation; margin may be overstated."
		).format(zero_cost)
...
def get_pi_progress_report(
	...
	for pi in pis:
		...
		advance_bank = (
			total_advance * (flt(pi["advance_pct"]) / 100.0) if pi["advance_pct"] else total_advance
		)
		...
		overall_pct = min(
			100.0, (advance_pct * 0.4) + (min(100.0, allocation_pct) * 0.3) + (min(100.0, pct_70) * 0.3)
		)
