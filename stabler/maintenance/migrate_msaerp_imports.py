...
def run_migration(company=None):
	...
	ci_map = {
		r["id"]: dict(r) for r in cur.execute("SELECT * FROM proforma_app_commercialinvoice").fetchall()
	}
	...
	for ci_id, sql_items in items_by_ci.items():
		...
		ci_name = (
			ci_norm_map.get(norm_raw)
			or frappe.db.get_value("Commercial Invoice", {"ci_number": raw_num}, "name")
			or frappe.db.get_value("Commercial Invoice", {"name": raw_num}, "name")
		)
		if not ci_name:
			continue
	...
	frappe.db.commit()
	print(
		f"Commercial Invoice Items Migration Complete: Created/Rebuilt {ci_items_created} items across {len(items_by_ci)} CIs."
	)
	...
	for c in containers_sqlite:
		...
		if ci_row:
			...
			ci_name = frappe.db.get_value(
				"Commercial Invoice", {"ci_number": ci_number}, "name"
			) or frappe.db.get_value("Commercial Invoice", {"name": ci_number}, "name")
		...
		doc.supplier = supplier or (
			frappe.db.get_value("Commercial Invoice", ci_name, "supplier") if ci_name else None
		)
	...
	for t in trucks_sqlite:
		...
		doc.company = company
		doc.commercial_invoice = ci_name
		doc.trucking_company = (
			str(t["trucking_company_id"]) if t["trucking_company_id"] else "Standard Transport"
		)
