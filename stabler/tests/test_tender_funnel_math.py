...
class ClassifyTest(unittest.TestCase):
	def test_precedence_result_beats_everything(self):
		...
		facts = {"result": "won", "submitted": True, "has_pricing": True, "sq_count": 5, "go_no_go": "go"}
	...
	def test_malformed_facts_degrade_to_seen_not_progress(self):
		self.assertEqual(
			f.classify({"result": "WON!", "sq_count": "abc" if False else 0, "go_no_go": "maybe"}), "seen"
		)
...
class SoBucketTest(unittest.TestCase):
	def test_board_stages_fold_into_story_buckets(self):
		out = f.summarise_so(["New", "Procurement", "Delivery", "Acceptance", "Invoicing", "Paid", "Closed"])
