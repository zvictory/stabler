...
def _install_fakes() -> None:
	...
	stabler_settings = types.ModuleType(
...
def setUpModule():
	...
	agreement_mod = importlib.import_module(
		"stabler.stabler.doctype.vehicle_agreement.vehicle_agreement"
	)
...
class TestScheduleVersionInvariants(unittest.TestCase):
	def setUp(self):
		...
		_DOCS.clear()
		_DOCS[
			("Vehicle Agreement", "VFA-2026-00001")
		] = _FakeDoc(
			company="ACME",
			currency="USD",
...
class TestPaymentApplicationInvariants(unittest.TestCase):
	...
	def _application(self, **overrides):
		...
		fields.update(overrides)
		return _instance(
			application_mod.VehicleFinancePaymentApplication, **fields
		)
...
class TestFollowUpLogAppendOnly(unittest.TestCase):
	def _log(self, **overrides):
		fields = dict(company="ACME", agreement="VFA-2026-00001", contact_type="Call", contact_result="Promise")
		fields.update(overrides)
