...
SPA_FILE = os.path.join(
	os.path.dirname(os.path.dirname(__file__)), "public", "js", "pages", "tender", "OperationsDesk.vue"
)
...
class TestTenderDeskSpaSource(unittest.TestCase):
	...
	def test_no_table_striped_class(self):
		self.assertNotIn(
			"table-striped", self.source, "OperationsDesk.vue must not add table-striped manually"
		)
	...
	def test_uses_request_token_pattern(self):
		self.assertIn(
			"reqToken", self.source, "OperationsDesk.vue must use request-token pattern for race safety"
		)
