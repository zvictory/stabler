...
HALF = HalfDayPolicy(
	min_worked_min=240, anchor_day_h=12, anchor_night_h=12, anchor_office_h=12, anchor_light_h=12
)
...
class ClassifyTest(unittest.TestCase):
	def test_out_of_employment_pre_hire(self):
		self.assertEqual(
			classify_day(
				rec("09:00", "18:00"), "DAY", False, LATE, HALF, NIGHT, "2026-04-01", "2026-05-01", None
			),
			"out_of_employment",
		)
	...
	def test_holiday(self):
		self.assertEqual(
			classify_day(
				rec("09:00", "18:00"), "DAY", True, LATE, HALF, NIGHT, "2026-05-09", "2026-01-01", None
			),
			"holiday",
		)
	...
	def test_absent_no_punch(self):
		self.assertEqual(
			classify_day(rec(None, None), "DAY", False, LATE, HALF, NIGHT, "2026-05-10", "2026-01-01", None),
			"absent",
		)
	...
	def test_absent_under_4h(self):
		...
		self.assertEqual(
			classify_day(
				rec("09:00", "12:30"), "DAY", False, LATE, HALF, NIGHT, "2026-05-10", "2026-01-01", None
			),
			"absent",
		)
	...
	def test_half_day_entry_after_anchor(self):
		...
		self.assertEqual(
			classify_day(
				rec("13:00", "18:00"), "DAY", False, LATE, HALF, NIGHT, "2026-05-10", "2026-01-01", None
			),
			"half_day",
		)
		...
		late, night, _half, mwp = policies_from_ruleset(
			{
				...
				"flat_fee_uzs": 20000,
				"night_start_hour": 21,
				"ot_method": "DAILY_HOURS",
				"min_worked_for_present_min": 200,
			}
		)
