...
class TestStatusCounts(unittest.TestCase):
    def test_fills_all_buckets(self):
        counts = rules.status_counts(
            [{"status": "BOOKED", "count": 3}, {"status": "AVAILABLE", "count": 1}]
        )
...
class TestFilterClauses(unittest.TestCase):
    ...
    def test_ci_all_filters(self):
        clauses, params = rules.ci_filter_clauses(
            search="ABC", status="BOOKED", supplier="SUP-1"
        )
...
class TestCountQuery(unittest.TestCase):
    def test_assembles_count(self):
        sql = rules.count_query("`tabImport Container` c", "c.company = %(company)s AND c.status = %(status)s")
        self.assertEqual(
            sql,
...
class TestDeriveBillCategory(unittest.TestCase):
    ...
    def test_transport_from_item(self):
        self.assertEqual(
            rules.derive_bill_category(item_codes=["Cross-Border Transport"]), "transport"
        )
...
class TestDerivePoLifecycle(unittest.TestCase):
    ...
    def test_draft(self):
        ...
        self.assertEqual(
            rules.derive_po_lifecycle(docstatus=0, advance_paid=500, per_received=50), "DRAFT"
        )
    ...
    def test_advance_paid(self):
        self.assertEqual(
            rules.derive_po_lifecycle(docstatus=1, advance_paid=1200), "ADVANCE_PAID"
        )
    ...
    def test_shipping_from_ci_status(self):
        self.assertEqual(
            rules.derive_po_lifecycle(
                docstatus=1, advance_paid=1200, ci_statuses=["IN_TRANSIT"]
            ),
            "SHIPPING",
        )
    ...
    def test_partial_delivery_is_shipping(self):
        self.assertEqual(
            rules.derive_po_lifecycle(
                docstatus=1, ci_statuses=["DELIVERED_TO_UZBEKISTAN", "IN_TRANSIT"]
            ),
            "SHIPPING",
        )
...
class TestImportOrderFilterClauses(unittest.TestCase):
    ...
    def test_pi_group_ignored_when_column_absent(self):
        clauses, params = rules.import_order_filter_clauses(
            pi_group="IPG-1", has_pi_group_col=False
        )
        self.assertEqual(clauses, [])
...
class TestImportOrderKpis(unittest.TestCase):
    def _rows(self):
        return [
            {"agreed_total": 1000, "docs_total": 800, "cash_difference": 200,
             "total_boxes": 100, "total_kg": 2000},
            {"agreed_total": 500, "docs_total": 400, "cash_difference": 100,
             "total_boxes": 50, "total_kg": 1000},
        ]

    def test_aggregates(self):
        k = rules.import_order_kpis(
            self._rows(), invoices_total=3, invoices_pending=1, invoices_done=2
        )
