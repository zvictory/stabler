...
class TestDeltas(unittest.TestCase):
	def test_kirim_multi(self):
		p = {
			"op": "kirim",
			"legs": [
				{"kassa": "nakit", "amount": 2_000_000},
				{"kassa": "pk", "amount": 3_000_000},
	...
	def test_konv_buy(self):
		...
		self.assertEqual(
			compute_deltas(p), [{"kassa": "nakit", "delta": -6_450_000.0}, {"kassa": "usd", "delta": 500.0}]
		)
...
class TestFlowKonv(unittest.TestCase):
	def test_buy_usd_button_dir_then_amount_rate(self):
		...
		self.assertEqual(
			act["deltas"], [{"kassa": "nakit", "delta": -6_450_000.0}, {"kassa": "usd", "delta": 500.0}]
		)
	...
	def test_same_currency_transfer_via_konv(self):
		...
		self.assertEqual(
			act["deltas"], [{"kassa": "nakit", "delta": -2_000_000.0}, {"kassa": "pk", "delta": 2_000_000.0}]
		)
	...
	def test_sell_usd_to_card_with_dollar_sign(self):
		...
		_reply, _kb, st, act = handle(st, BTN_CONFIRM, CTX)
		self.assertEqual(
			act["deltas"], [{"kassa": "usd", "delta": -100.0}, {"kassa": "pk", "delta": 1_260_000.0}]
		)
