...
class UnbilledReceiptsDraftGuard(FrappeTestCase):
	...
	def test_a_partly_billed_receipt_can_still_be_billed_for_the_remainder(self):
		...
		"""
		pr = self._receipt()
		submitted = self._bill(pr.name)
		frappe.get_doc("Purchase Invoice", submitted).db_set("docstatus", 1)
		...
		remainder = create_purchase_invoice_from_pr(pr.name)
		self.assertNotEqual(remainder["name"], submitted)
	...
	def test_the_page_asks_once_for_every_receipt_it_shows(self):
		"""One batched query, and an empty page must not reach the DB at all.

		``IN ()`` is a syntax error rather than an empty match, so the early
		return is load-bearing, not an optimisation.
		"""
		...
		self.assertEqual(
			_draft_invoices_by_receipt([first.name, second.name]),
			{first.name: drafted},
