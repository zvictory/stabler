...
| **Landed** | `Σ (PO.base_grand_total + Σ charges.amount)` — a **post-win** figure. Not the quotation's pre-win landed estimate, which is what a sourcing user actually enters (§1). Renders **unconditionally** — never replaced by the estimate, matching DirectorBoard's own `landed` (P1-1, coordinator review, 2026-09-02) |
| **Pre-win estimate** | `CRM Deal.custom_bid_pricing.landed_goods` — the fixed number a sourcing officer typed to price the bid (Zafar's pre-win costing rule, §1). A sub-line inside the Landed cell, shown whenever it is nonzero, **independent of PO count or docstatus** — P1-2 measured that gating it on PO existence made it vanish the moment an empty draft PO was raised, restoring M16's original 0-shown defect |
...
| M7 | The screen uses the house layer | **passes** (2026-09-02) — `ds-panel`/`ds-table`/`ds-chip` throughout, same risk-tone map DirectorBoard uses (`MyTenders.vue:161`) |
| M8 | The active-stage chip is shown only while that filter is in effect | **passes** (2026-09-02) — `stageChipLabel` (`MyTenders.vue:114`) keys off `funnelDeals` (resolved), not `funnelStage` (intent) alone, so it is silent before the second request answers and after it fails (S2) |
| M9 | The screen says whether it is showing everything or only your assignments | **passes** (2026-09-02) — `scopeSentence` (`MyTenders.vue:74`) reads the endpoint's own `oversight` field (`tender.py:2557`); says nothing until the first load answers. Its 2 strings are reported translated into all five catalogues elsewhere (coordinator review, 2026-09-02); this worktree does not carry that change, so `make check`'s i18n gate here still lists them — not touched, per instruction (see report) |
...
| M11 | A won or lost tender is distinguishable from an open one | **passes** (2026-09-02) — `resultTone`/`resultLabel` (`MyTenders.vue:177-178`) render a chip guarded on `r.result`, now including `pending` (`RESULT_TONE`, `MyTenders.vue:176`; P2-6, coordinator review — `_clean_intake` normalizes to won/lost/pending, `tender.py:1512`, so pending is real here too). `result` itself is gated server-side on submitted evidence — `evidence["lifecycle"]["submitted"]`, the same flag `_has_submission_evidence` gives DirectorBoard (`tender.py:2538`) — so a result set without a submitted bid arrives as `""` and the row falls back to DirectorBoard's own amber "Unverified" chip (`MyTenders.vue:221`) instead of showing an unverifiable result (P1-3, coordinator review, 2026-09-02; S6) |
| M12 | A failed load is distinguishable from "no tenders match these filters" | **passes** (2026-09-02) — `load()` records the failure on `error` (not only the toast) and clears it before the next attempt; the empty-state footer renders one message for a failed load and a different one for an honest empty result. P1-4 (coordinator review, 2026-09-02): `error` latches across `useAutoRefresh`'s background ticks and clears only at the top of the next `load()`, so a stale failure sitting over rows that loaded fine earlier could mislabel them. The footer guard is now `error && !data.rows.length` (`MyTenders.vue:239`), not `error` alone — a background failure over real rows renders the honest filtered/empty copy, and load-OK-then-fail-then-filtered-to-zero still shows "No tenders match these filters.", never the load-failure copy |
...
| M14 | The table scrolls on a phone; the page does not | **passes** (2026-09-02) — `.mt-scroll` wraps the table with `overflow-x: auto` (`MyTenders.vue:261`), but that alone does not scroll: `.ds-table` is `width: 100%` (`stabler-modernist.css:389`, shared, not edited), so with nothing to overflow the table just shrinks — measured live against `DirectorBoard.vue`'s identical `.board-scroll`, which does not scroll. Fix is a `min-width` on each `<th>` (700px total), not `.ds-table` itself. Source-level only; no jsdom in this repo, so rendered/phone behaviour is **not** verified (see report) |
| M15 | An auto-refreshing screen says how fresh it is | **passes** (2026-09-02) — `generated_at` |
| M16 | The landed figure shown to a sourcing user is one they can act on before the win | **passes** (2026-09-02; redesigned 2026-09-02 per coordinator review P1-1/P1-2) — the Landed cell (`MyTenders.vue:224-229`) now renders both figures together instead of switching between them: `r.landed`, the post-win PO sum, unconditionally and matching DirectorBoard's own `landed` with no switch (so the two screens can no longer disagree, P1-1); and `r.landed_estimate`, the pre-win estimate a sourcing officer types onto the deal's own bid pricing (`_deal_landed_estimate`, `tender.py:1077`, reading `CRM Deal.custom_bid_pricing.landed_goods` — Zafar's pre-win costing rule, `00-SETUP.md`), as a sub-line guarded only on its own truthiness — never on PO count or docstatus, which is what let an empty draft PO blank the estimate under the old switch (P1-2). `rowLanded` no longer exists. Source-level proof only; real seeded-data correctness needs `make test-bench` (see report) |
