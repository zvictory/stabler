...
def seed():
	...
	if not frappe.db.exists("Item", item_code):
		...
		it.update(
			{
				"item_code": item_code,
				"item_name": "Vagon buksa podshipnigi (demo lot)",
	...
	deal.update(
		{"organization": _org(f"{MARK} UTY lot 2026-4417 — vagon buksa podshipniklari"), "company": company}
	)
	...
	pricing = tender_api.save_deal_bid_pricing(
		deal.name,
		json.dumps(
			{
	...
	frappe.db.commit()
	print(
		f"3. evaluation: cheapest sourced cost {sourced_cost:.2f} "
	...
	so.update(
		{
			...
			"custom_crm_deal": deal.name,
			"custom_board_stage": "Procurement",
		}
	)
	so.append(
		"items",
		{"item_code": item_code, "qty": QTY, "rate": bid_price / QTY, "delivery_date": add_days(today(), 45)},
	)
...
def verify():
	...
	ik = r.get("intake") or {}
	check(
		"1  intake carries the lot + go decision",
	...
	check(
		"2b two countries — RU+CN (policy >= 2)",
		q["countries"] >= 2 and q["has_2_countries"],
	...
	check(
		"2d exactly one Cheapest row, and it is Ningbo (CN)",
	...
	sourced = 104.0 * 40.0
	check(
		"3a bid pricing anchored on the sourced cost (landed_goods = cheapest)",
		abs(flt(pnl.get("landed_goods")) - sourced) < 0.01,
		f"landed_goods={pnl.get('landed_goods')} (sourced {sourced})",
	)
	check(
		"3b engine back-solved a bid price above cost",
	...
	check(
		"4  submission audit is server-stamped",
		bool(ik.get("submitted_at"))
		and bool(ik.get("submitted_by"))
	...
	check(
		"5a result=won with audit trail, at the price sourcing produced",
		ik.get("result") == "won"
		and bool(ik.get("result_at"))
	...
	check(
		"5b contract SO visible on the board (stage Procurement)",
	...
	check(
		"6a deal-tagged PO on the control board",
		pb["kpi"]["po_count"] >= 1,
		f"po_count={pb['kpi']['po_count']}",
	)
	check(
		"6b vendor comparison (compare) rows present",
		len(pb.get("compare") or []) >= 1,
...
def seed_portfolio():
	...
	for stage, count, (lo, hi) in PLAN:
		for _ in range(count):
			...
			deal = frappe.new_doc("CRM Deal")
			deal.update(
				{"organization": _org(f"{MARK} UTY lot 2026-{lot_no} — {subject}"), "company": company}
			)
			deal.insert(ignore_permissions=True)
			frappe.db.set_value(
				"CRM Deal", deal.name, "creation", f"{created} 09:00:00", update_modified=False
			)
			_register("CRM Deal", deal.name)

			intake = {
				"lot_no": f"UZEX-2026-{lot_no}",
				"buyer": buyer,
				"unit": "dona",
				"volume": qty,
			...
			if stage in ("priced", "submitted", "won", "lost", "done"):
				pricing = tender_api.save_deal_bid_pricing(
					...
					json.dumps(
						{
							"mode": "margin",
							"margin_pct": random.choice([12.0, 15.0, 18.0]),
...
def verify_portfolio():
	...
	check(
		"policy-gap badge > 0",
		r["meta"]["sourcing_policy_gap"] >= 1,
		f"got {r['meta']['sourcing_policy_gap']}",
	)
	check(
		"funnel last rung == stages won (lost not counted)",
