...
- **A plausible number is more dangerous than an implausible one.** "the other 15
  bench modules" was challenged; "the 84 frappe-free modules" sat unchallenged in
...
undoing? Does a pattern for this already exist in this repo? (Check before
inventing: the ratchet, the pinned-version file, the derived-not-hardcoded list
are all already here.) What does this decision make harder later?
...
|---|---|---|
| **Silent skip** | exit 0 | `make check` skipping two sub-gates when `node_modules` is absent; a bench module skipping 3 of 3 tests and printing OK |
| **Silent truncation** | a complete-looking list | `bd ready` returning 10 of 50 by default; `head -10` reported as a total |
| **Stale value** | a confident number | a "no matches" baseline that was 17 matches on remeasurement; module counts wrong by 2.1× and 3.3× |
...
before anything may proceed — the repo already learned this for lint:
*"a permanently red hook is one everybody bypasses with `--no-verify`."*
